...
public class WebConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new LoginCheckInterceptor()) //인터셉터로 등록할 클래스
                ...
                .excludePathPatterns("/createCookie","smtp.**","/member/mailConfirm","/question/list","/cart/change","/cart/saved","/item/**","/member/duplicate-check-userId","/", "/member/save", "/member/login", "/member/logout", "/member/kakao","/item/main", "/js/**", "/css/**", "/jsp/**","/style/**", "/error", "/img/**","/product/**", "/*.ico", "/favicon/**","/upload/**"); //예외할 주소, 로그인 안해도 사용 가능
            }
        }
